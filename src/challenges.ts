...
export function getRegionRoleCount(userRoles: string[]): number {
  const regionRoles = ['Asgarnia', 'Kandarin', 'Fremennik', 'Desert', 'Morytania', 'Tirannwn', 'Wilderness', 'Kourend', 'Varlamore'];
...
export function getChallengeCardEligibility(difficulty: string): number {
   ...
   */
  export function getChallengeCardModel(difficulty: string):
   ...
   */
  export async function loadChallengeMain(userId: string): Promise<ChallengeMain | null> {
   ...
   */
  export async function updateChallengeMain(userId: string,updateData: Partial<ChallengeMain>,): Promise<void> {
   ...
   */
  export async function loadChallengeCard(userId: string, difficulty: string): Promise<ChallengeCardBase | null> {
    ...
    return (await ChallengeCardModel.findOne({ where: { user_id: userId } })) as ChallengeCardBase | null;
  }
   ...
   */
  export async function saveChallengeCard(userId: string, difficulty: string, challenges: string[], rerolled: number ): Promise<void> {
    const ChallengeCardModel = getChallengeCardModel(difficulty);
    if (!ChallengeCardModel) {
      throw new Error(`Invalid difficulty tier: ${difficulty}`);
    }
  ...
  export function existingChallengesToList(existingChallenges: ChallengeCardBase, difficulty: string): string[] {
  ...
  export function getNextDifficultyTier(currentTier: string): string {
    ...
    return currentIndex < tiers.length - 1 ? tiers[currentIndex + 1] : 'Grandmaster';
  }
   ...
   */
  export function generateNewChallenges(difficulty: string, userRoles: string[]): string[] {
   ...
   */
  function getEligibleChallenges(difficulty: string, userRoles: string[]): string[] {
    return challengesData.challenges
      .filter((challenge: { difficulty: string; region1: string; region2?: string }) => {
        ...
        if (challenge.region2) {
          return userRoles.includes(challenge.region1) && userRoles.includes(challenge.region2);
        }
